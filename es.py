...
def population_update(master, params):
    ...
    for i in range(params['num_agents']):
        ...
        #results是一个列表，其中每个元素是每个worker收集的数据
    ...
    if params['w_nov'] > 0:
        ...
        # 若此时div这一项的权重为零则不需要计算dets
        dets = np.zeros(np.array(rollout_rewards).shape)
        # 100*2
    #data 维度 （500，2，1000，3）
