...
def main():
    ...
    parser.add_argument('--flush', '-fl', type=int,
                        default=1000)  # may need this. it resets ray, because sometimes it fills the memory.
    parser.add_argument('--ob_filter', '-ob', type=str, default='MeanStdFilter')  # 'NoFilter'
    parser.add_argument('--w_nov', '-wn', type=float,
                        default=-1)  # if negative it uses the adaptive method, else it will be fixed at the value you pick (0,1). Note that if you pick 1 itll be unsupervised (ie no reward)
    ...
    parser.add_argument('--moea_select', '-mo', type=int, default=1)  # if use the moea_selection
    ...
    master.dist_population = {0: 0.10914541334067529, 1: 0.13438793610344651, 2: 0.11487448669384792, 3: 0.1348667746871593, 4: 0.10295657634914812, 5: 0.06983221172204705, 6: 0.09798568730286648, 7: 0.0657356898173213, 8: 0.10433567323023724, 9: 0.06587955075325085}

    master.reward_population = {0: 28.26704493155401, 1: 24.49380771616392, 2: 29.95568447584943, 3: 27.16066100270101, 4: 29.281972847322603, 5: 25.657021475765916, 6: 24.492700004960707, 7: 29.002286217907407, 8: 25.35292549466718, 9: 27.574207839405087}

    print(master.moea_selection(params))
